...
1. Call `get_accounting_skill_schema` with `skill_id=accounts-payable-reconciliation-th`;
   validate inputs and use only the returned result contract.
...
Use supplied external evidence only; when supplier-payment or statement evidence is missing,
stop and request a connect-or-upload fallback. Never infer missing transactions.
