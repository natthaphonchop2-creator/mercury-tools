...
description: Use when the user asks to reconcile accounts payable, supplier bills, payments, or expense records
---
...
`skill_id=accounts-payable-reconciliation-th`, then call `run_accounting_skill` with the same
...
2. Call `search_erp_actions` for supplier bills, expenses, credit notes, and payment evidence.
...
4. Check host-reported external MCP capabilities for the supplier-payment, statement, or
   evidence source. Stop and request a connect-or-upload fallback if a required capability is
   absent.
