...
alter table public.mercury_provider_capability_qualifications
  add column if not exists public_output_field_paths jsonb;
...
begin
  if not exists (
    ...
    where conname = 'mercury_provider_capability_public_output_paths_shape'
      and conrelid =
        'public.mercury_provider_capability_qualifications'::regclass
  ) then
    alter table public.mercury_provider_capability_qualifications
      add constraint mercury_provider_capability_public_output_paths_shape
      check (
        public_output_field_paths is null
        or jsonb_typeof(public_output_field_paths) = 'array'
      );
  end if;
end;
$$;
...
declare
  ...
  public_path_values text[];
  sorted_public_paths text[];
...
begin
  if jsonb_typeof(p_qualification) <> 'object'
    or exists (
      ...
      where key not in (
        ...
        'schema_hash', 'response_shape_hash', 'required_permissions',
        'capability_version_sha256', 'qualification_state', 'company_sha256',
        'evidence_revision_sha256', 'qualification_evidence_uri',
  ...
  from jsonb_array_elements_text(candidate.required_permissions)
    with ordinality as item(value, ordinal);
  if permission_values is distinct from sorted_permissions
    or cardinality(permission_values)
      <> cardinality(array(select distinct unnest(permission_values)))
    or exists (
  ...
  if candidate.public_output_field_paths is not null then
    if jsonb_typeof(candidate.public_output_field_paths) <> 'array'
      or exists (
        select 1 from jsonb_array_elements(candidate.public_output_field_paths) as item
        where jsonb_typeof(item) <> 'string'
      ) then
      raise exception 'mercury_provider_capability_public_output_paths_invalid';
    end if;
    select array_agg(value order by ordinal), array_agg(value order by value)
      into public_path_values, sorted_public_paths
    from jsonb_array_elements_text(candidate.public_output_field_paths)
      with ordinality as item(value, ordinal);
    if public_path_values is distinct from sorted_public_paths
      or cardinality(public_path_values)
        <> cardinality(array(select distinct unnest(public_path_values)))
      or exists (
        select 1 from unnest(public_path_values) as value
        where value !~ '^/(\*|([^~/*]|~[01])+)(/(\*|([^~/*]|~[01])+))*$'
      ) then
      raise exception 'mercury_provider_capability_public_output_paths_invalid';
    end if;
  ...
  version_payload := jsonb_build_object(
  ...
  if candidate.public_output_field_paths is not null then
    version_payload := version_payload || jsonb_build_object(
      'public_output_field_paths', candidate.public_output_field_paths
    );
  end if;
  expected_version_hash := encode(digest(
    public.mercury_canonical_jsonb(version_payload), 'sha256'
  ), 'hex');
  ...
  if not found then
    if candidate.public_output_field_paths is null
      or candidate.qualification_state <> 'discovered_unreviewed'
  ...
  if (
    ...
    candidate.public_output_field_paths, candidate.schema_hash,
    candidate.response_shape_hash, candidate.required_permissions,
    ...
    existing.public_output_field_paths, existing.schema_hash,
    existing.response_shape_hash, existing.required_permissions,
  ...
  if not (
    (
      existing.qualification_state = 'discovered_unreviewed'
      and candidate.qualification_state = 'schema_validated'
    )
    or (
      existing.qualification_state = 'schema_validated'
      and candidate.qualification_state = 'nonproduction_qualified'
    )
    or (
      existing.qualification_state = 'nonproduction_qualified'
      and candidate.qualification_state = 'enabled'
    )
    or (
      existing.qualification_state = 'enabled'
      and candidate.qualification_state in ('disabled', 'superseded')
    )
  ) then
  ...
  if existing.qualification_state = 'nonproduction_qualified'
    ...
    and (
      ...
      or candidate.nonproduction_evidence_revision_sha256
        is distinct from existing.nonproduction_evidence_revision_sha256
      or candidate.nonproduction_company_sha256
        is distinct from existing.nonproduction_company_sha256
    ) then
  ...
  if candidate.qualification_state in ('nonproduction_qualified', 'enabled') then
    if jsonb_typeof(p_artifact) <> 'object'
      ...
      or candidate.qualification_evidence_uri <> (
        ...
        || candidate.capability_version_sha256 || '-'
        || candidate.evidence_revision_sha256 || '.json'
      )
    ...
    expected_artifact_revision := encode(digest(
      public.mercury_canonical_jsonb(p_artifact - 'evidence_revision_sha256'),
      'sha256'
    ), 'hex');
    if p_artifact->>'evidence_revision_sha256' is distinct from expected_artifact_revision
      ...
      or p_artifact->>'capability_version_sha256'
        is distinct from candidate.capability_version_sha256
      ...
      or (p_artifact->>'evaluated_at')::timestamptz
        is distinct from candidate.evidence_evaluated_at
      or (p_artifact->>'evidence_expires_at')::timestamptz
        is distinct from candidate.evidence_expires_at
      or coalesce((p_artifact->>'passed')::boolean, false) is not true then
      raise exception 'mercury_provider_capability_evidence_invalid';
    end if;
  elsif candidate.qualification_state = 'schema_validated' then
    if candidate.company_sha256 is not null
      or candidate.evidence_revision_sha256 is not null
      or candidate.qualification_evidence_uri is not null
      or candidate.evidence_evaluated_at is not null
      or candidate.evidence_expires_at is not null
  ...
  if candidate.environment = 'production'
    and candidate.qualification_state in (
      'nonproduction_qualified', 'enabled', 'disabled', 'superseded'
    ) then
    ...
    where evidence.provider = candidate.provider
      ...
      and evidence.public_output_field_paths
        is not distinct from candidate.public_output_field_paths
      ...
      and evidence.evidence_revision_sha256
        = candidate.nonproduction_evidence_revision_sha256
  ...
  if candidate.qualification_state = 'enabled' then
    if not (
      ...
      or candidate.normalized_capability
        ~ '^documents\.[a-z][a-z0-9_]*\.create$'
    ) then
...
revoke all on function public.publish_mercury_provider_capability_qualification(
  jsonb,
  jsonb
) from public, anon, authenticated;
grant execute on function public.publish_mercury_provider_capability_qualification(
  jsonb,
  jsonb
) to service_role;
