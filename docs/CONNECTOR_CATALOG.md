...
| FlowAccount | API driver | `reviewed` | `production`, `sandbox` | User through hidden local CLI input; advanced local runtime | Reviewed catalog actions are `not_validated` until safe environment evidence exists | 2026-07-19 |
| PEAK Accounting | API driver | `reviewed` | `production`, `uat`, `sandbox` | User through hidden local CLI input; advanced local runtime | Reviewed catalog actions are `not_validated` until safe environment evidence exists | 2026-07-19 |
...
- **API driver**: reviewed reads and mutations run only through a separately connected
  advanced local Mercury MCP. Secrets stay in repository-local secure state.
- **Local Bridge**: a customer-operated bridge is required for LAN, desktop, export, or
  other local-system access. The hosted service never receives LAN credentials or raw
  database access.
